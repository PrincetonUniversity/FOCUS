...
if rank == 0:
    ...
    master = False
test = FOCUS(comm=comm, extension="ellipse", verbose=True)

maxiter = 100
test.globals.cg_maxiter = maxiter
# standard execution
...
collections = (
    ...
    "nelder-mead",
    "powell",
)  #'dogleg', 'trust-ncg', 'trust-constr', 'trust-krylov', 'trust-exact',
...
for method in collections:
    ...
    test.callback(res.x)
    if master:
        ax[0].semilogy(test.globals.evolution[:, 1], label=method)
        ax[1].semilogy(
            test.globals.evolution[:, 0], test.globals.evolution[:, 1], label=method
        )
