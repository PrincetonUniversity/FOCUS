...
#------------------------------------------  7  ----------------------------------------------------
def plot_coils(coil_data, plottype='filament', color=(0,0,1), Ncolor=1, \
                   width=2.0, recW=0.1, recH=0.1, opacity=1.0, WindSurf=None, EageOn=False):
    '''
    ...
    if plottype == 'filament' :
        if Ncolor > 1:
            ...
            for icoil in coil_data:
                mlab.plot3d(icoil.x, icoil.y, icoil.z, color = c[ic%Ncolor], line_width=width, opacity=opacity)
                ic += 1
        else :
            for icoil in coil_data:
                mlab.plot3d(icoil.x, icoil.y, icoil.z, color = color, line_width=width, opacity=opacity)
    elif plottype == 'real' :
        if Ncolor > 1:
            c = color_array(Ncolor)
            ic = 0
            for icoil in coil_data:
                xsurf, ysurf, zsurf = expand(icoil.x, icoil.y, icoil.z, recW, recH, WindSurf)
                mlab.mesh(xsurf, ysurf, zsurf, color=c[ic%Ncolor], opacity=opacity)
                if EageOn :
                    for ii in range(4):
            ...
            for icoil in coil_data:
                xsurf, ysurf, zsurf = expand(icoil.x, icoil.y, icoil.z, recW, recH, WindSurf)
                mlab.mesh(xsurf, ysurf, zsurf, color=color, opacity=opacity)
                if EageOn :
                    for ii in range(4):
                        mlab.plot3d(xsurf[ii,:], ysurf[ii,:], zsurf[ii,:], color=(0,0,0), line_width=width, opacity=opacity)
                #for ii in range(4):
...
def vmec2focus(vmec_file, focus_file='plasma.boundary', bnorm_file = None, curpol = 1.0, old=True):
    ...
    with open(focus_file, 'w') as fofile:
        ...
        for imn in range(mn):
            fofile.write("{:4d}  {:4d} \t {:12.5E}  {:12.5E}  {:12.5E}  {:12.5E} \n".format(xn[imn], xm[imn], rbc[imn], 0.0, 0.0, zbs[imn]))
        fofile.write("#Bn harmonics curpol= {:12.5E} \n".format(curpol))
        fofile.write('# n m bnc bns'+'\n')
        if old :
            for imn in range(Nbnf):
                fofile.write("{:d} \t {:d} \t {:12.5E} \t {:12.5E} \n".format(-bn[imn], bm[imn], 0.0, bns[imn])) 
                #egative n, because FOCUS and BNORM are taking different rep.
        else :
            for imn in range(Nbnf):
                fofile.write("{:d} \t {:d} \t {:12.5E} \t {:12.5E} \n".format(-bn[imn], bm[imn], 0.0, -bns[imn])) #negative sign for coef, since New FOCUS is addinng the Bn.
                #egative n, because FOCUS and BNORM are taking different rep.           
