...
!------------------------------------------------------------------------------------------------------   
  ...
  REAL, dimension(0:Nzeta-1)            :: psi_diff
  ...
  if( ideriv >= 0 ) then
     ...
     do jzeta = 0, Nzeta - 1
        ...
        lsum  = lsum + lflux
        psi_diff(jzeta) = lflux - target_tflux
        dflux = dflux + psi_diff(jzeta)**2
     enddo ! end do jzeta
  ...
  if ( ideriv >= 1 ) then
     ...
     call MPI_REDUCE(ldF, dF, Ndof, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr )
     RlBCAST( dF, Ndof, 0 )
