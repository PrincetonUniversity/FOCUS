SUBROUTINE poinplot
  ...
  !------------------------------------------------------------------------------------------------------ 
  USE famus_globals, only : dp, myid, ncpu, zero, half, pi, pi2, ounit, pi, sqrtmachprec, pp_maxiter, &
...
SUBROUTINE find_axis(RZ, MAXFEV, XTOL)
  USE famus_globals, only : dp, myid, ounit, zero, pp_phi, Nfp_raw
  USE mpi
  IMPLICIT NONE
...
SUBROUTINE axis_fcn(n,x,fvec,iflag)
  USE famus_globals, only : dp, myid, IsQuiet, ounit, zero, pi2, sqrtmachprec, pp_phi, Nfp_raw, pp_xtol
  USE mpi
  IMPLICIT NONE
...
SUBROUTINE BRpZ( t, x, dx )
  ...
  ! dZ/dphi = BZ / Bphi
  !----------------------
  use famus_globals, only : dp, zero, ounit, myid, ierr, myworkid, nworker, MPI_COMM_MYWORLD
  USE MPI
  implicit none
...
SUBROUTINE BRpZ_iota( t, x, dx )
  ...
  ! dZ/dphi = BZ / Bphi
  !----------------------
  use famus_globals, only : dp, zero, ounit, myid, ierr
  USE MPI
  implicit none
