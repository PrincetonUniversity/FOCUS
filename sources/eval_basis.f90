...
SUBROUTINE enforce_periodicity(icoil)
    ...
    do iter_pos = 0,N-1
	if(eval_points(iter_pos)>=CPCoil(icoil)%vect(7)) then
		CPCoil(icoil)%basis_3(iter_pos,0:2)=CPCoil(icoil)%basis_3(iter_pos,CPCoil(icoil)%NCP-3:CPCoil(icoil)%NCP-1)
		...
		CPCoil(icoil)%db_dt(iter_pos,0:2)  =CPCoil(icoil)%db_dt(iter_pos,CPCoil(icoil)%NCP-3:CPCoil(icoil)%NCP-1)
		CPCoil(icoil)%db_dt_2(iter_pos,0:2)=CPCoil(icoil)%db_dt_2(iter_pos,CPCoil(icoil)%NCP-3:CPCoil(icoil)%NCP-1)
