...
C	
       DIMENSION X(N),Y(N),G(N),H(N,N),D(N),INDEX(N)
       LOGICAL MODHES,PRNT
C
C  ***** INITIALIZATION *****
C
       ...
       CURF=FUNCT(N,X)
       CALL DERIVS(N,X,G,H)
	 PRINT*,'STARTING FUNCTION VALUE',CURF
	DO 1000 I=1,N
...
C
	 ...
	 CALL INSTEP(N,X,D,GAMMA,S,PRNT)
         IFLAG=1
	 PRINT*,'HESSIAN MODIFIED,  INITIAL STEPSIZE =',S
...
30     CONTINUE
       CALL DERIVS(N,X,G,H)
	PRINT*,'# OF STEPSIZE REDUCTIONS ',NRED
	PRINT*,'NEW FUNCTION VALUE = ',CURF
       DO 50 I=1,N
...
40     CONTINUE
       IF (MAXGRAD.LT.EPS) THEN
         PRINT*,' GRADIENT TOLERANCE ATTAINED; OPTIMIZATION COMPLETED'
         RETURN
       ELSE
         PRINT*,'SQUARED NORM OF THE GRADIENT =',MAXGRAD
         GO TO 10
       END IF
       END
...
C      AFTER INTERCHANGES
       ...
       LOGICAL MODHES
       DIMENSION H(N,N),TEMP(N),INDEX(N)
C
C  **** INITIALIZATION ****
       MODHES=.FALSE.
       MAXDIAG=0.0
...
C     SOLUTION TO THE SYSTEM HD=G
      ...
      DIMENSION H(N,N),G(N),D(N),DF(N),GI(N),INDEX(N)
